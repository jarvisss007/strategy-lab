...
2026-09-19 · clean (6/6 families in discover.py's FAMILIES match REGISTRY.md; 35 configs, config set identical to 09-12; no commit has touched discover.py or the fetchers since 09-01) · **NO DRIFT ATTRIBUTABLE TO NEW DATA — sixth consecutive week.** Weekend run (MARKET: CLOSED 2026-09-19): discoveries.csv and this log are verdict/counter books that do not compound (cleared by the 09-08 audit), nothing was entered or queued. (1) PIPELINE, the Saturday-vs-Monday scheduling mismatch for the sixth straight week, and this run is its cleanest demonstration yet. The Monday 09-14 00:37 fetch ran correctly and returned data through Friday 09-11 — byte-for-byte the extent last week's manual Saturday refetch already reached. Unlike 09-05 and 09-12, this run followed the task file's step order literally and ran discover.py BEFORE refetching: that first pass reproduced the entire 2026-09-12 block on **34 of 35 config rows byte-for-byte**, the 35th moving 0.01 (illiquidity q0.2 net 0.89 → 0.90, an adjusted-close revision, not a session). A Saturday run on a Monday panel is arithmetic on last week's data — measured, not argued. Panels then refetched: 2011-09-19..2026-09-18, 3,775 rows, 145 cols (prices 148), `failed: []`. discoveries.csv therefore holds TWO 2026-09-19 blocks — **use the second** (bench 1.25). (2) Attribution by the 08-22 method (refreshed panel truncated back to the 09-11 end): **the 5 new sessions (09-14..09-18) moved every config's net Sharpe by ≤0.02 and OOS by ≤0.11 (max: post-quadwitch-week OOS −0.12 → −0.23); all six family verdicts, DSRs and PBOs are identical on the truncated panel.** Every week-over-week move above 0.2 is the 5-row front roll re-phasing the positional `np.arange(len(score)) // H` grid diagnosed 08-15: high-volume-premium q0.1 H21 net +0.22 → −0.42 (new sessions +0.01), q0.2 H21 +0.11 → −0.45 (+0.02), confluence-fade q0.2 H21 −1.09 → −1.50 (+0.01), value-area-fade q0.1 H21 −0.06 → −0.40 (−0.01), illiquidity q0.2 OOS 1.31 → 1.03 (−0.03). One category difference vs the FROZEN row: Price-action value-area fade real-but-loses → **dead** (best config identity q0.2 H21 → q0.1 H5, net −0.05 → −0.21, DSR 0.21 → 0.11, PBO 0.51 → 0.62) — dead on the truncated panel too, so front roll again, and the 08-15 withdrawal stands. Worth one honest note in its favour: this is the first week all six of its configs print negative (span −0.21..−0.42) rather than straddling zero, and q0.1 H21 at −0.40 sits just past the bottom of its 08-15 phase span (−0.38..+0.14) — suggestive, still not established while the phase defect is unfixed. Rest: overnight structural-but-uncostable (basket gross 1.50, net −0.42) · opex real-but-loses, DSR 0.91 → 0.92, PBO 0.60 → 0.63, best still post-opex-week · turn-of-month t5 real-but-loses, DSR 1.0, PBO 0.05 → 0.06, net 1.13 vs bench 1.25 — not a SURVIVOR, so no MC drawdown run · microstructure real-but-loses, illiquidity q0.2 DSR 0.15 → 0.18, PBO 0.05 → 0.00 · confluence dead, best config q0.1 H21 → q0.1 H10. Nothing crossed DSR 0.5 or 0.95. SUGGESTIONS awaiting sign-off under rule 1, none actioned: (a) anchor `reb` to a calendar epoch + report H-config Sharpes as a median across phases (08-15, six weeks of evidence); (b) refresh panels inside the sentinel run or fetch Friday post-close (08-22; done by hand six weeks running); (c) the one-token survivor-filter fix at discover.py:297 (09-12); (d) declare the sentinel's books non-session-unit in the task text (09-12); (e) NEW — the task file's step 2 says "run discover.py" with no refetch step, which is exactly what produced this week's duplicate block; fold the refetch into step 2 so the order is written down rather than remembered. · zdte 45/60 (+5), recorder continuous through SPY_2026-09-18.csv, 15 sessions to the B-L density gate ≈ 2026-10-09 at 5/week · crypto 76 derived minute-days 07-05..09-18 (+7); the spec's `research/data/BTC-USD_*.csv` counter reads 1 (rotate.py gzips, 30 .csv.gz) — honest source research/minutes/. learned_weights.json **FOSSIL for the sixth straight week** (mtime 2026-08-11, built from 38 days; 38 accrued days unincorporated — the unincorporated tail now equals the sample it was built on); frozen figures: skill −0.031, edge_found=false, hit 49.12% vs naive 49.98%, gross −0.195 bps/trade against 60 bps costs · insider rolling feed 400 = the KEEP=400 cap (window 2026-09-10..09-18), saturated and unable to express accrual; deep-sample events.json 1,062, untouched since 2026-07-07, so the "double to 2,124" milestone stays unfireable as instrumented · no milestone crossed
